...
type: docs
weight: 70
url: /id/python-net/aspose.threed.entities/trimesh/from_mesh/
is_root: false
---
...
# Definisikan deklarasi vertex sebagai {FVector3 Position; FVector3 Normal; FVector2 UV}
...
# konversi mesh menjadi tri-mesh menggunakan tata letak memori yang ditentukan
mesh = Sphere().to_mesh()
triMesh = TriMesh.from_mesh(vd, mesh)
# simpan ke aliran, 115 vertex * 32 byte per vertex
with open("output.bin", "wb") as stream:
    triMesh.write_vertices_to(stream)
    # simpan indeks sebagai ushort ke aliran, 504 indeks * 2 byte per indeks
