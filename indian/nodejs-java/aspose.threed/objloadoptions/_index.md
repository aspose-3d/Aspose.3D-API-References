---
title: "ObjLoadOptions"
second_title: "Aspose.3D के लिए Node.js के माध्यम से Java API संदर्भ"
description: 
type: docs

url: /hi/nodejs-java/aspose.threed/objloadoptions/
---
## ObjLoadOptions class

wavefront obj के लिए लोड विकल्प


## विधियाँ

### constructor{#constructor}

| नाम | विवरण |
| --- | --- |
| constructor() | ObjLoadOptions का कंस्ट्रक्टर |

 **Result:**



---


### getFlipCoordinateSystem{#getFlipCoordinateSystem}

| नाम | विवरण |
| --- | --- |
| getFlipCoordinateSystem() | इम्पोर्ट करते समय नियंत्रण बिंदुओं/नॉर्मल के कोऑर्डिनेट सिस्टम को उलटना है या नहीं, प्राप्त या सेट करता है। |

 **Result:**



---


### setFlipCoordinateSystem{#setFlipCoordinateSystem}

| नाम | विवरण |
| --- | --- |
| setFlipCoordinateSystem(value) | इम्पोर्ट करते समय नियंत्रण बिंदुओं/नॉर्मल के कोऑर्डिनेट सिस्टम को उलटना है या नहीं, प्राप्त या सेट करता है। |

 **Result:**



---


### getEnableMaterials{#getEnableMaterials}

| नाम | विवरण |
| --- | --- |
| getEnableMaterials() | प्रत्येक ऑब्जेक्ट के लिए सामग्री आयात करनी है या नहीं, प्राप्त या सेट करता है। |

 **Result:**



---


### setEnableMaterials{#setEnableMaterials}

| नाम | विवरण |
| --- | --- |
| setEnableMaterials(value) | प्रत्येक ऑब्जेक्ट के लिए सामग्री आयात करनी है या नहीं, प्राप्त या सेट करता है। |

 **Result:**



---


### getScale{#getScale}

| नाम | विवरण |
| --- | --- |
| getScale() | x/y/z अक्ष पर स्केल करता है, डिफ़ॉल्ट मान 1.0 है। |

 **Result:**



---


### setScale{#setScale}

| नाम | विवरण |
| --- | --- |
| setScale(value) | x/y/z अक्ष पर स्केल करता है, डिफ़ॉल्ट मान 1.0 है। |

 **Result:**



---


### getNormalizeNormal{#getNormalizeNormal}

| नाम | विवरण |
| --- | --- |
| getNormalizeNormal() | लोडिंग के दौरान नॉर्मल वेक्टर को सामान्यीकृत करना है या नहीं, प्राप्त या सेट करता है। डिफ़ॉल्ट मान true है। |

 **Result:**



---


### setNormalizeNormal{#setNormalizeNormal}

| नाम | विवरण |
| --- | --- |
| setNormalizeNormal(value) | लोडिंग के दौरान नॉर्मल वेक्टर को सामान्यीकृत करना है या नहीं, प्राप्त या सेट करता है। डिफ़ॉल्ट मान true है। |

 **Result:**



---


### getFileFormat{#getFileFormat}

| नाम | विवरण |
| --- | --- |
| getFileFormat() | वर्तमान Save/Load विकल्प में निर्दिष्ट फ़ाइल फ़ॉर्मेट को प्राप्त करता है। |

 **Result:**



---


### getEncoding{#getEncoding}

| नाम | विवरण |
| --- | --- |
| getEncoding() | टेक्स्ट-आधारित फ़ाइलों के लिए डिफ़ॉल्ट एन्कोडिंग को प्राप्त करता है या सेट करता है। डिफ़ॉल्ट मान null है, जिसका अर्थ है कि इम्पोर्टर/एक्सपोर्टर यह तय करेगा कि कौन सी एन्कोडिंग उपयोग की जाएगी। |

 **Result:**



---


### getFileSystem{#getFileSystem}

| नाम | विवरण |
| --- | --- |
| getFileSystem() | लोड/सेव के दौरान बाहरी निर्भरताओं को कैसे प्रबंधित किया जाए, इसे उपयोगकर्ता को संभालने की अनुमति देता है। |

 **Result:**



---


### setFileSystem{#setFileSystem}

| नाम | विवरण |
| --- | --- |
| setFileSystem(value) | लोड/सेव के दौरान बाहरी निर्भरताओं को कैसे प्रबंधित किया जाए, इसे उपयोगकर्ता को संभालने की अनुमति देता है। |

 **Result:**



---


### getLookupPaths{#getLookupPaths}

| नाम | विवरण |
| --- | --- |
| getLookupPaths() | कुछ फ़ाइलें जैसे OBJ बाहरी फ़ाइल पर निर्भर करती हैं, लुकअप पाथ्स Aspose.3D को लोड करने के लिए बाहरी फ़ाइल खोजने की अनुमति देते हैं। |

 **Result:**



---


### getFileName{#getFileName}

| नाम | विवरण |
| --- | --- |
| getFileName() | एक्सपोर्ट/इम्पोर्ट सीन की फ़ाइल नाम। यह वैकल्पिक है, लेकिन OBJ की सामग्री जैसी बाहरी एसेट्स को सीरियलाइज़ करते समय उपयोगी है। |

 **Result:**



---


### setFileName{#setFileName}

| नाम | विवरण |
| --- | --- |
| setFileName(value) | एक्सपोर्ट/इम्पोर्ट सीन की फ़ाइल नाम। यह वैकल्पिक है, लेकिन OBJ की सामग्री जैसी बाहरी एसेट्स को सीरियलाइज़ करते समय उपयोगी है। |

 **Result:**



---



