---
title: GetKeyframeSequence
second_title: Aspose.3D for.NETAPIリファレンス
description: 指定したアニメーション インスタンスのキーフレーム シーケンスを取得します
type: docs
weight: 50
url: /ja/net/aspose.threed/property/getkeyframesequence/
---
## Property.GetKeyframeSequence method

指定したアニメーション インスタンスのキーフレーム シーケンスを取得します。

```csharp
public KeyframeSequence GetKeyframeSequence(AnimationNode anim, bool create)
```

| パラメータ | タイプ | 説明 |
| --- | --- | --- |
| anim | AnimationNode | キーフレーム シーケンスを作成するアニメーション。 |
| create | Boolean | キーフレーム シーケンスが見つからない場合は作成します。 |

### 戻り値

指定されたアニメーション インスタンスのキーフレーム シーケンス

### 関連項目

* class [KeyframeSequence](../../../aspose.threed.animation/keyframesequence/)
* class [AnimationNode](../../../aspose.threed.animation/animationnode/)
* class [Property](../)
* 名前空間 [Aspose.ThreeD](../../property/)
* 組み立て [Aspose.3D](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.3D.dll -->
