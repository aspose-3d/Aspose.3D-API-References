---
title: VertexElementVertexCrease
second_title: Aspose.3D for.NETAPIリファレンス
description: 指定したコンポーネントの頂点の折り目を定義します
type: docs
weight: 940
url: /ja/net/aspose.threed.entities/vertexelementvertexcrease/
---
## VertexElementVertexCrease class

指定したコンポーネントの頂点の折り目を定義します

```csharp
public class VertexElementVertexCrease : VertexElementDoublesTemplate
```

## コンストラクター

| 名前 | 説明 |
| --- | --- |
| [VertexElementVertexCrease](vertexelementvertexcrease/)() | の新しいインスタンスを初期化します`VertexElementVertexCrease`class. |

## プロパティ

| 名前 | 説明 |
| --- | --- |
| [Data](../../aspose.threed.entities/vertexelementdoublestemplate/data/) { get; } | 頂点データを取得 |
| [Indices](../../aspose.threed.entities/vertexelement/indices/) { get; } | インデックスを取得します data |
| [MappingMode](../../aspose.threed.entities/vertexelement/mappingmode/) { get; set; } | 要素のマッピング方法を取得または設定します。 |
| [Name](../../aspose.threed.entities/vertexelement/name/) { get; set; } | 名前を取得または設定します。 |
| [ReferenceMode](../../aspose.threed.entities/vertexelement/referencemode/) { get; set; } | 要素の参照方法を取得または設定します。 |
| [VertexElementType](../../aspose.threed.entities/vertexelement/vertexelementtype/) { get; } | のタイプを取得します[`VertexElement`](../vertexelement/) |

## メソッド

| 名前 | 説明 |
| --- | --- |
| override [Clear](../../aspose.threed.entities/vertexelementdoublestemplate/clear/)() | 直接およびインデックス配列からすべての要素を削除します。 |
| [CopyTo](../../aspose.threed.entities/vertexelementdoublestemplate/copyto/)(VertexElementDoublesTemplate) | 指定した要素にデータをコピーします |
| [SetData](../../aspose.threed.entities/vertexelementdoublestemplate/setdata/)(double[]) | データ読み込み |
| [SetIndices](../../aspose.threed.entities/vertexelement/setindices/)(int[]) | インデックスを読み込む |
| override [ToString](../../aspose.threed.entities/vertexelement/tostring/)() | 頂点要素の文字列表現. |

### 関連項目

* class [VertexElementDoublesTemplate](../vertexelementdoublestemplate/)
* 名前空間 [Aspose.ThreeD.Entities](../../aspose.threed.entities/)
* 組み立て [Aspose.3D](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.3D.dll -->
