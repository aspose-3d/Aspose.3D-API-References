...
type: docs
weight: 70
url: /ja/python-net/aspose.threed.entities/trimesh/from_mesh/
is_root: false
---
...
# 頂点宣言を {FVector3 Position; FVector3 Normal; FVector2 UV} のように定義する
...
# 指定されたメモリレイアウトを使用してメッシュをトライメッシュに変換する
mesh = Sphere().to_mesh()
triMesh = TriMesh.from_mesh(vd, mesh)
# ストリームに保存する、115 頂点 × 1 頂点あたり 32 バイト
with open("output.bin", "wb") as stream:
    triMesh.write_vertices_to(stream)
    # インデックスを ushort としてストリームに保存する、504 インデックス × 1 インデックスあたり 2 バイト
