...
description: "点群はトポロジ情報を含まず、制御点と頂点要素のみが含まれます。"
type: docs
weight: 132
url: /ja/java/com.aspose.threed/pointcloud/
---
...
点群はトポロジ情報を含まず、制御点と頂点要素のみが含まれます。
## コンストラクタ

| コンストラクタ | 説明 |
| --- | --- |
| [PointCloud(String name)](#PointCloud-java.lang.String-) | [PointCloud](../../com.aspose.threed/pointcloud) のコンストラクタ |
| [PointCloud()](#PointCloud--) | [PointCloud](../../com.aspose.threed/pointcloud) のコンストラクタ |
## メソッド

| メソッド | 説明 |
| --- | --- |
| [<T>getDeformers2()](#-T-getDeformers2--) | 指定されたデフォーマータイプのすべてのデフォーマーを取得します |
| [addElement(VertexElement element)](#addElement-com.aspose.threed.VertexElement-) | 既存の頂点要素を現在のジオメトリに追加します |
| [createElement(VertexElementType type)](#createElement-com.aspose.threed.VertexElementType-) | 指定されたタイプの頂点要素を作成し、ジオメトリに追加します。 |
| [createElement(VertexElementType type, MappingMode mappingMode, ReferenceMode referenceMode)](#createElement-com.aspose.threed.VertexElementType-com.aspose.threed.MappingMode-com.aspose.threed.ReferenceMode-) | 指定されたタイプの頂点要素を作成し、ジオメトリに追加します。 |
...
| [fromGeometry(Geometry g)](#fromGeometry-com.aspose.threed.Geometry-) | ジオメトリオブジェクトから新しい PointCloud インスタンスを作成する |
| [fromGeometry(Geometry g, int density)](#fromGeometry-com.aspose.threed.Geometry-int-) | ジオメトリオブジェクトから新しいポイントクラウドインスタンスを作成します。 |
...
| [getDimension()](#getDimension--) | ポイントクラウドに次元値が存在する場合、それは組織化されたポイントクラウドであることを示します。 |
...
| [setDimension(Vector2 value)](#setDimension-com.aspose.threed.Vector2-) | ポイントクラウドに次元値が存在する場合、それは組織化されたポイントクラウドであることを示します。 |
...
[PointCloud](../../com.aspose.threed/pointcloud) のコンストラクタ

**Parameters:**
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| 名前 | java.lang.String | このエンティティの名前 |
...
指定されたデフォーマータイプのすべてのデフォーマーを取得します
...
ジオメトリオブジェクトから新しい PointCloud インスタンスを作成する
...
ジオメトリオブジェクトから新しいポイントクラウドインスタンスを作成します。密度は単位三角形あたりのポイント数です（単位三角形はメッシュから最大表面積を持つ三角形です）。

**Parameters:**
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| g | [Geometry](../../com.aspose.threed/geometry) | メッシュまたはその他のジオメトリインスタンス |
| 密度 | int | 単位三角形あたりのポイント数 |
...
ポイントクラウドに次元値が存在する場合、それは組織化されたポイントクラウドであることを示します。サイズが指定されていない場合は、非組織化ポイントクラウドとみなされます。組織化ポイントクラウドとは、画像のような構造を持つことを意味します。
