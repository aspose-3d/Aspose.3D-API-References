...
type: docs
weight: 70
url: /nl/python-net/aspose.threed.entities/trimesh/from_mesh/
is_root: false
---
...
# Definieer een vertex-declaratie als {FVector3 Position; FVector3 Normal; FVector2 UV}
...
# converteer een mesh naar een tri-mesh met behulp van de gespecificeerde geheugenindeling
mesh = Sphere().to_mesh()
triMesh = TriMesh.from_mesh(vd, mesh)
# sla het op in een stream, 115 vertices * 32 bytes per vertex
with open("output.bin", "wb") as stream:
    triMesh.write_vertices_to(stream)
    # sla indexen op als ushort in de stream, 504 indexen * 2 bytes per index
