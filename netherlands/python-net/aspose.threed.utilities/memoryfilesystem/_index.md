...
type: docs
weight: 130
url: /nl/python-net/aspose.threed.utilities/memoryfilesystem/
is_root: false
---
...
# maak een scène met materiaal
scene = Scene()
scene.root_node.create_child_node(Box()).material = LambertMaterial()
# maak een opslaan-optie en specificeer het bestandssysteem, zodat het afhankelijke bestand naar het geheugen wordt geschreven
opt = FileFormat.WAVEFRONT_OBJ.create_save_options()
mfs = MemoryFileSystem()
opt.file_system = mfs
# de bestandsnaam van het materiaal van obj is gekoppeld aan de bestandsnaam van obj, dus we hebben een expliciete naam nodig.
opt.file_name = "test.obj"
with BytesIO() as ms:
    scene.save(ms, opt)
# de test.obj werd geschreven naar variabele ms, en we kunnen ook de inhoud van het test.mtl-bestand verkrijgen door
