...
type: docs
weight: 70
url: /es/python-net/aspose.threed.entities/trimesh/from_mesh/
is_root: false
---
...
# Definir una declaración de vértice como {FVector3 Position; FVector3 Normal; FVector2 UV}
...
# convertir una malla a tri-malla usando la disposición de memoria especificada
mesh = Sphere().to_mesh()
triMesh = TriMesh.from_mesh(vd, mesh)
# guardarlo en un flujo, 115 vértices * 32 bytes por vértice
with open("output.bin", "wb") as stream:
    triMesh.write_vertices_to(stream)
    # guardar índices como ushort en el flujo, 504 índices * 2 bytes por índice
