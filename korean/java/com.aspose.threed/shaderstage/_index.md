---
title: "ShaderStage"
second_title: "Aspose.3D for Java API 레퍼런스"
description: "셰이더 단계"
type: docs
weight: 168
url: /ko/java/com.aspose.threed/shaderstage/
---

**Inheritance:**
java.lang.Object
```
public final class ShaderStage
```

셰이더 단계
## 생성자

| 생성자 | 설명 |
| --- | --- |
| [ShaderStage()](#ShaderStage--) |  |
## 필드

| 필드 | 설명 |
| --- | --- |
| [COMPUTE_SHADER](#COMPUTE-SHADER) | 컴퓨트 셰이더 |
| [FRAGMENT_SHADER](#FRAGMENT-SHADER) | 프래그먼트 셰이더 |
| [GEOMETRY_SHADER](#GEOMETRY-SHADER) | 지오메트리 셰이더 |
| [VERTEX_SHADER](#VERTEX-SHADER) | 버텍스 셰이더 |
## 메서드

| 메서드 | 설명 |
| --- | --- |
| [equals(Object arg0)](#equals-java.lang.Object-) |  |
| [getClass()](#getClass--) |  |
| [hashCode()](#hashCode--) |  |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [toString()](#toString--) |  |
| [wait()](#wait--) |  |
| [wait(long arg0)](#wait-long-) |  |
| [wait(long arg0, int arg1)](#wait-long-int-) |  |
### ShaderStage() {#ShaderStage--}
```
public ShaderStage()
```


### COMPUTE_SHADER {#COMPUTE-SHADER}
```
public static final int COMPUTE_SHADER
```


컴퓨트 셰이더

### FRAGMENT_SHADER {#FRAGMENT-SHADER}
```
public static final int FRAGMENT_SHADER
```


프래그먼트 셰이더

### GEOMETRY_SHADER {#GEOMETRY-SHADER}
```
public static final int GEOMETRY_SHADER
```


지오메트리 셰이더

### VERTEX_SHADER {#VERTEX-SHADER}
```
public static final int VERTEX_SHADER
```


버텍스 셰이더

### equals(Object arg0) {#equals-java.lang.Object-}
```
public boolean equals(Object arg0)
```




**Parameters:**
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
boolean
### getClass() {#getClass--}
```
public final native Class<?> getClass()
```




**Returns:**
java.lang.Class<?>
### hashCode() {#hashCode--}
```
public native int hashCode()
```




**Returns:**
int
### notify() {#notify--}
```
public final native void notify()
```




### notifyAll() {#notifyAll--}
```
public final native void notifyAll()
```




### toString() {#toString--}
```
public String toString()
```




**Returns:**
java.lang.String
### wait() {#wait--}
```
public final void wait()
```




### wait(long arg0) {#wait-long-}
```
public final void wait(long arg0)
```




**Parameters:**
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| arg0 | long |  |

### wait(long arg0, int arg1) {#wait-long-int-}
```
public final void wait(long arg0, int arg1)
```




**Parameters:**
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| arg0 | long |  |
| arg1 | int |  |

