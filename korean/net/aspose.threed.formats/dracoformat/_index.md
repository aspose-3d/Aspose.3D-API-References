---
title: DracoFormat
second_title: .NET API 참조용 Aspose.3D
description: Google Draco 형식
type: docs
weight: 1090
url: /ko/net/aspose.threed.formats/dracoformat/
---
## DracoFormat class

Google Draco 형식

```csharp
public class DracoFormat : FileFormat
```

## 속성

| 이름 | 설명 |
| --- | --- |
| [CanExport](../../aspose.threed/fileformat/canexport/) { get; } | Aspose.3D가 현재 파일 형식으로 장면 내보내기를 지원하는지 여부를 가져옵니다. |
| [CanImport](../../aspose.threed/fileformat/canimport/) { get; } | Aspose.3D가 현재 파일 형식에서 장면 가져오기를 지원하는지 여부를 가져옵니다. |
| [ContentType](../../aspose.threed/fileformat/contenttype/) { get; } | 파일 형식 콘텐츠 가져오기 type |
| [Extension](../../aspose.threed/fileformat/extension/) { get; } | 이 유형의 확장 이름을 가져옵니다. |
| [Extensions](../../aspose.threed/fileformat/extensions/) { get; } | 이 유형의 확장 이름을 가져옵니다. |
| [FileFormatType](../../aspose.threed/fileformat/fileformattype/) { get; } | 파일 형식 가져오기 type |
| [Version](../../aspose.threed/fileformat/version/) { get; } | 파일 형식 version 가져오기 |

## 행동 양식

| 이름 | 설명 |
| --- | --- |
| [CreateLoadOptions](../../aspose.threed/fileformat/createloadoptions/)() | 이 파일 format 에 대한 기본 로드 옵션을 만듭니다. |
| [CreateSaveOptions](../../aspose.threed/fileformat/createsaveoptions/)() | 이 파일 format 에 대한 기본 저장 옵션을 만듭니다. |
| [Decode](../../aspose.threed.formats/dracoformat/decode/#decode)(byte[]) | 메모리 data 에서 포인트 클라우드 또는 메쉬를 디코딩합니다. |
| [Decode](../../aspose.threed.formats/dracoformat/decode/#decode_1)(string) | 지정된 파일 name 에서 포인트 클라우드 또는 메쉬를 디코딩합니다. |
| [Encode](../../aspose.threed.formats/dracoformat/encode/#encode)(Entity, DracoSaveOptions) | 엔터티를 Draco 원시 데이터 로 인코딩합니다. |
| [Encode](../../aspose.threed.formats/dracoformat/encode/#encode_1)(Entity, Stream, DracoSaveOptions) | 엔터티를 지정된 stream 로 인코딩합니다. |
| [Encode](../../aspose.threed.formats/dracoformat/encode/#encode_2)(Entity, string, DracoSaveOptions) | 엔터티를 지정된 file 로 인코딩합니다. |
| override [ToString](../../aspose.threed/fileformat/tostring/)() | string 로 포맷 |

### 또한보십시오

* class [FileFormat](../../aspose.threed/fileformat/)
* 네임스페이스 [Aspose.ThreeD.Formats](../../aspose.threed.formats/)
* 집회 [Aspose.3D](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.3D.dll -->
