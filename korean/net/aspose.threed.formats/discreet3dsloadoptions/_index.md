---
title: Discreet3dsLoadOptions
second_title: .NET API 참조용 Aspose.3D
description: 3DS 파일에 대한 로드 옵션.
type: docs
weight: 1060
url: /ko/net/aspose.threed.formats/discreet3dsloadoptions/
---
## Discreet3dsLoadOptions class

3DS 파일에 대한 로드 옵션.

```csharp
public class Discreet3dsLoadOptions : LoadOptions
```

## 생성자

| 이름 | 설명 |
| --- | --- |
| [Discreet3dsLoadOptions](discreet3dsloadoptions/)() | 의 생성자`Discreet3dsLoadOptions` |

## 속성

| 이름 | 설명 |
| --- | --- |
| [ApplyAnimationTransform](../../aspose.threed.formats/discreet3dsloadoptions/applyanimationtransform/) { get; set; } | 애니메이션 트랙의 첫 번째 프레임에 정의된 변환 사용 여부를 가져오거나 설정합니다. |
| [Encoding](../../aspose.threed.formats/ioconfig/encoding/) { get; set; } | 텍스트 기반 파일의 기본 인코딩을 가져오거나 설정합니다. 기본값은 가져오기/내보내기가 사용할 인코딩을 결정한다는 의미인 null입니다. |
| [FileFormat](../../aspose.threed.formats/ioconfig/fileformat/) { get; } | 현재 저장/로드 옵션에 지정된 파일 형식을 가져옵니다. |
| [FileName](../../aspose.threed.formats/ioconfig/filename/) { get; set; } | 내보내기/가져오기 장면의 파일 이름입니다. 선택 사항이지만 OBJ의 재질과 같은 외부 자산을 직렬화할 때 유용합니다. |
| [FileSystem](../../aspose.threed.formats/ioconfig/filesystem/) { get; set; } | 사용자가 로드/저장 중에 외부 종속성을 관리하는 방법을 처리할 수 있습니다. |
| [FlipCoordinateSystem](../../aspose.threed.formats/discreet3dsloadoptions/flipcoordinatesystem/) { get; set; } | 가져오기/내보내기 중에 제어점/법선의 반전 좌표계를 가져오거나 설정합니다. |
| [GammaCorrectedColor](../../aspose.threed.formats/discreet3dsloadoptions/gammacorrectedcolor/) { get; set; } | 3ds 파일에는 동일한 속성에 대해 원래 색상과 감마 보정된 색상이 포함될 수 있습니다. |
| [LookupPaths](../../aspose.threed.formats/ioconfig/lookuppaths/) { get; set; } | OBJ와 같은 일부 파일은 외부 파일에 의존하며 조회 경로를 통해 Aspose.3D는 로드할 외부 파일을 찾을 수 있습니다. |

### 또한보십시오

* class [LoadOptions](../loadoptions/)
* 네임스페이스 [Aspose.ThreeD.Formats](../../aspose.threed.formats/)
* 집회 [Aspose.3D](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.3D.dll -->
