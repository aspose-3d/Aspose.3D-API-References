...
description: 모양은 제어점 세트의 변형을 설명하며 Maya의 클러스터 디포머와 유사합니다. 예를 들어 생성된 형상에 모양을 추가할 수 있습니다. 그리고 형상과 기하학은 동일한 토폴로지 정보를 갖지만 제어점의 위치는 다릅니다. 다양한 영향력으로 지오메트리가 변형 효과를 냅니다.
type: docs
weight: 640
url: /ko/net/aspose.threed.entities/shape/
---
## Shape class

모양은 제어점 세트의 변형을 설명하며 Maya의 클러스터 디포머와 유사합니다. 예를 들어 생성된 형상에 모양을 추가할 수 있습니다. 그리고 형상과 기하학은 동일한 토폴로지 정보를 갖지만 제어점의 위치는 다릅니다. 다양한 영향력으로 지오메트리가 변형 효과를 냅니다.
...
| 이름 | 설명 |
| --- | --- |
| [Shape](shape/#constructor)() | 의 새 인스턴스를 초기화합니다.`Shape` 클래스. |
| [Shape](shape/#constructor_1)(string) | 의 새 인스턴스를 초기화합니다.`Shape` 클래스. |
...
| [Indices](../../aspose.threed.entities/shape/indices/) { get; } | 인덱스를 가져옵니다. |
...
| 이름 | 설명 |
| --- | --- |
| static [FromControlPoints](../../aspose.threed.entities/shape/fromcontrolpoints/)(params Vector3[]) | 기본 색인을 사용하여 지정된 제어점으로 모양을 만듭니다. |
