...
type: docs
weight: 70
url: /ko/python-net/aspose.threed.entities/trimesh/from_mesh/
is_root: false
---
...
# 정점 선언을 {FVector3 Position; FVector3 Normal; FVector2 UV}와 같이 정의합니다.
...
# 지정된 메모리 레이아웃을 사용하여 메시를 삼각형 메쉬로 변환합니다.
mesh = Sphere().to_mesh()
triMesh = TriMesh.from_mesh(vd, mesh)
# 스트림에 저장합니다, 115개의 정점 × 정점당 32바이트
with open("output.bin", "wb") as stream:
    triMesh.write_vertices_to(stream)
    # 인덱스를 ushort 형식으로 스트림에 저장합니다, 504개의 인덱스 × 인덱스당 2바이트
