...
description: Базовый класс всех визуализируемых геометрических объектов таких какMesh./meshNurbsSurface./nurbssurfacePatch./patchи т. д..
type: docs
weight: 360
...
Базовый класс всех визуализируемых геометрических объектов (таких как[`Mesh`](../mesh),[`NurbsSurface`](../nurbssurface),[`Patch`](../patch)и т. д.).

Базовый класс[`Geometry`](../geometry)поддерживает: &lt;ul&gt;&lt;li&gt; **Управление контрольными точками** , контрольные точки определяют базовую трехмерную пространственную структуру геометрии, разные геометрические типы имеют разные способы определения конкретных трехмерных моделей . &lt;/li&gt;&lt;li&gt; **Определение элемента вершины** , элементы вершины применяют к геометрии дополнительную информацию, такую как нормали/координаты UV/цвета вершин, см.[`VertexElement`](../vertexelement)подробнее.&lt;/li&gt;&lt;li&gt; **Деформация объекта** ,[`Deformer`](../../aspose.threed.deformers/deformer)может быть связан с анимацией геометрической формы.&lt;/li&gt;&lt;/ul&gt;
...
| Имя | Описание |
| --- | --- |
| [Geometry](geometry)(string) | Инициализирует новый экземпляр класса[`Geometry`](../geometry). |
...
| [CreateElementUV](../../aspose.threed.entities/geometry/createelementuv#createelementuv)(TextureMapping) | Создает[`VertexElementUV`](../vertexelementuv)с заданным типом наложения текстуры. |
| [CreateElementUV](../../aspose.threed.entities/geometry/createelementuv#createelementuv_1)(TextureMapping, MappingMode, ReferenceMode) | Создает[`VertexElementUV`](../vertexelementuv)с заданным типом наложения текстуры. |
