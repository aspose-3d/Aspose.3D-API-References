---
title: "ShaderStage"
second_title: "Справочник API Aspose.3D для Java"
description: "Этап шейдера"
type: docs
weight: 168
url: /ru/java/com.aspose.threed/shaderstage/
---

**Inheritance:**
java.lang.Object
```
public final class ShaderStage
```

Этап шейдера
## Конструкторы

| Конструктор | Описание |
| --- | --- |
| [ShaderStage()](#ShaderStage--) |  |
## Поля

| Поле | Описание |
| --- | --- |
| [COMPUTE_SHADER](#COMPUTE-SHADER) | Вычислительный шейдер |
| [FRAGMENT_SHADER](#FRAGMENT-SHADER) | Фрагментный шейдер |
| [GEOMETRY_SHADER](#GEOMETRY-SHADER) | Геометрический шейдер |
| [VERTEX_SHADER](#VERTEX-SHADER) | Вершинный шейдер |
## Методы

| Метод | Описание |
| --- | --- |
| [equals(Object arg0)](#equals-java.lang.Object-) |  |
| [getClass()](#getClass--) |  |
| [hashCode()](#hashCode--) |  |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [toString()](#toString--) |  |
| [wait()](#wait--) |  |
| [wait(long arg0)](#wait-long-) |  |
| [wait(long arg0, int arg1)](#wait-long-int-) |  |
### ShaderStage() {#ShaderStage--}
```
public ShaderStage()
```


### COMPUTE_SHADER {#COMPUTE-SHADER}
```
public static final int COMPUTE_SHADER
```


Вычислительный шейдер

### FRAGMENT_SHADER {#FRAGMENT-SHADER}
```
public static final int FRAGMENT_SHADER
```


Фрагментный шейдер

### GEOMETRY_SHADER {#GEOMETRY-SHADER}
```
public static final int GEOMETRY_SHADER
```


Геометрический шейдер

### VERTEX_SHADER {#VERTEX-SHADER}
```
public static final int VERTEX_SHADER
```


Вершинный шейдер

### equals(Object arg0) {#equals-java.lang.Object-}
```
public boolean equals(Object arg0)
```




**Parameters:**
| Параметр | Тип | Описание |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
boolean
### getClass() {#getClass--}
```
public final native Class<?> getClass()
```




**Returns:**
java.lang.Class<?>
### hashCode() {#hashCode--}
```
public native int hashCode()
```




**Returns:**
int
### notify() {#notify--}
```
public final native void notify()
```




### notifyAll() {#notifyAll--}
```
public final native void notifyAll()
```




### toString() {#toString--}
```
public String toString()
```




**Returns:**
java.lang.String
### wait() {#wait--}
```
public final void wait()
```




### wait(long arg0) {#wait-long-}
```
public final void wait(long arg0)
```




**Parameters:**
| Параметр | Тип | Описание |
| --- | --- | --- |
| arg0 | long |  |

### wait(long arg0, int arg1) {#wait-long-int-}
```
public final void wait(long arg0, int arg1)
```




**Parameters:**
| Параметр | Тип | Описание |
| --- | --- | --- |
| arg0 | long |  |
| arg1 | int |  |

