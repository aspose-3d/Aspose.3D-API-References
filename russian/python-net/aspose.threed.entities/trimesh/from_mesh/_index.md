...
type: docs
weight: 70
url: /ru/python-net/aspose.threed.entities/trimesh/from_mesh/
is_root: false
---
...
# Определите объявление вершины как {FVector3 Position; FVector3 Normal; FVector2 UV}
...
# преобразовать сетку в tri-mesh, используя указанную компоновку памяти
mesh = Sphere().to_mesh()
triMesh = TriMesh.from_mesh(vd, mesh)
# сохранить её в поток, 115 вершин * 32 байта на вершину
with open("output.bin", "wb") as stream:
    triMesh.write_vertices_to(stream)
    # сохранить индексы как ushort в поток, 504 индекса * 2 байта на индекс
