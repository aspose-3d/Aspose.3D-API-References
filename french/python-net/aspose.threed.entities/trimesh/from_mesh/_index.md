...
type: docs
weight: 70
url: /fr/python-net/aspose.threed.entities/trimesh/from_mesh/
is_root: false
---
...
# Définir une déclaration de sommet comme {FVector3 Position; FVector3 Normal; FVector2 UV}
...
# convertir un maillage en tri-maillage en utilisant la disposition mémoire spécifiée
mesh = Sphere().to_mesh()
triMesh = TriMesh.from_mesh(vd, mesh)
# l'enregistrer dans un flux, 115 sommets * 32 octets par sommet
with open("output.bin", "wb") as stream:
    triMesh.write_vertices_to(stream)
    # enregistrer les indices en tant que ushort dans le flux, 504 indices * 2 octets par indice
