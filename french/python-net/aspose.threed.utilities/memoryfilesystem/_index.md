...
type: docs
weight: 130
url: /fr/python-net/aspose.threed.utilities/memoryfilesystem/
is_root: false
---
...
# créer une scène avec un matériau
scene = Scene()
scene.root_node.create_child_node(Box()).material = LambertMaterial()
# créer une option d'enregistrement et spécifier le système de fichiers, afin que le fichier dépendant soit écrit en mémoire
opt = FileFormat.WAVEFRONT_OBJ.create_save_options()
mfs = MemoryFileSystem()
opt.file_system = mfs
# Le nom du fichier de matériau de l'obj est associé au nom du fichier obj, il nous faut donc un nom explicite.
opt.file_name = "test.obj"
with BytesIO() as ms:
    scene.save(ms, opt)
# Le test.obj a été écrit dans la variable ms, et nous pouvons également obtenir le contenu du fichier test.mtl par
