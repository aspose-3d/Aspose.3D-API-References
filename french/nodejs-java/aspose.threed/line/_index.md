...
Une polyline est un chemin défini par un ensemble de points avec Geometry.ControlPoints, et reliés par Segments, ce qui signifie qu’il peut également s’agir d’un ensemble de segments de ligne connectés.  La ligne est généralement un objet linéaire, ce qui signifie qu’elle ne peut pas être utilisée pour représenter une courbe ; pour représenter une courbe, utilisez NurbsCurve.
...
| Nom | Description |
| --- | --- |
| fromPoints(points) | Construit une instance de Line à partir d'un ensemble de points. |
