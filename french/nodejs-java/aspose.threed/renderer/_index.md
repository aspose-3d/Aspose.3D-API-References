...
Le contexte concernant le rendu.  @hideconstructor
...
| Nom | Description |
| --- | --- |
| getRenderFactory() | Obtient la fabrique permettant de créer des objets liés au rendu. |
...
| Nom | Description |
| --- | --- |
| getAssetDirectories() | Répertoires qui stockent les actifs externes |
...
| Nom | Description |
| --- | --- |
| getFrustum() | Obtient ou définit le frustum utilisé pour fournir la matrice de vue. |
...
| Nom | Description |
| --- | --- |
| setFrustum(value) | Obtient ou définit le frustum utilisé pour fournir la matrice de vue. |
...
| Nom | Description |
| --- | --- |
| getShader() | Obtient ou définit l'instance du shader utilisée pour le rendu de la géométrie. |
...
| Nom | Description |
| --- | --- |
| setShader(value) | Obtient ou définit l'instance du shader utilisée pour le rendu de la géométrie. |
...
| Nom | Description |
| --- | --- |
| getFallbackEntityRenderer() | Obtient ou définit le rendu d'entité de secours lorsque l'entité n'a aucun rendu spécial défini. |
...
| Nom | Description |
| --- | --- |
| setFallbackEntityRenderer(value) | Obtient ou définit le rendu d'entité de secours lorsque l'entité n'a aucun rendu spécial défini. |
...
| Nom | Description |
| --- | --- |
| clearCache() | Efface manuellement le cache. Aspose.3D met en cache certains objets comme les matériaux/géométries dans des types internes compatibles avec le pipeline de rendu. Cela doit être appelé manuellement lorsque la scène subit des changements majeurs. |
...
| Nom | Description |
| --- | --- |
| getPostProcessing(name) | Obtient un post-processeur intégré pris en charge par le moteur de rendu. |
...
| Nom | Description |
| --- | --- |
| execute(postProcessing, result) | Exécute un post-traitement sur la cible de rendu spécifiée. |
...
| Nom | Description |
| --- | --- |
| registerEntityRenderer(renderer) | Enregistre le renderer d'entité pour l'entité spécifiée |
