...
type: docs
weight: 70
url: /ar/python-net/aspose.threed.entities/trimesh/from_mesh/
is_root: false
---
...
# تعريف إعلان رأس كـ {FVector3 Position; FVector3 Normal; FVector2 UV}
...
# تحويل شبكة إلى شبكة ثلاثية باستخدام تخطيط الذاكرة المحدد
mesh = Sphere().to_mesh()
triMesh = TriMesh.from_mesh(vd, mesh)
# حفظها إلى تدفق، 115 رأسًا * 32 بايت لكل رأس
with open("output.bin", "wb") as stream:
    triMesh.write_vertices_to(stream)
    # حفظ الفهارس كـ ushort إلى التدفق، 504 فهرس * 2 بايت لكل فهرس
