...
type: docs
weight: 70
url: /sv/python-net/aspose.threed.entities/trimesh/from_mesh/
is_root: false
---
...
# Definiera en vertexdeklaration som {FVector3 Position; FVector3 Normal; FVector2 UV}
...
# konvertera ett mesh till tri‑mesh med angiven minneslayout
mesh = Sphere().to_mesh()
triMesh = TriMesh.from_mesh(vd, mesh)
# spara det till en ström, 115 vertexar * 32 byte per vertex
with open("output.bin", "wb") as stream:
    triMesh.write_vertices_to(stream)
    # spara index som ushort till ström, 504 index * 2 byte per index
