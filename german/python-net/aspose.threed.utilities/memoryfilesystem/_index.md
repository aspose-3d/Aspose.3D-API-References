...
type: docs
weight: 130
url: /de/python-net/aspose.threed.utilities/memoryfilesystem/
is_root: false
---
...
# Erstelle eine Szene mit Material
scene = Scene()
scene.root_node.create_child_node(Box()).material = LambertMaterial()
# Erstelle eine Speicheroption und gib das Dateisystem an, sodass die abhängige Datei in den Speicher geschrieben wird
opt = FileFormat.WAVEFRONT_OBJ.create_save_options()
mfs = MemoryFileSystem()
opt.file_system = mfs
# Der Materialdateiname von obj ist mit dem Dateinamen von obj verknüpft, daher benötigen wir einen expliziten Namen.
opt.file_name = "test.obj"
with BytesIO() as ms:
    scene.save(ms, opt)
# Die test.obj wurde in die Variable ms geschrieben, und wir können den Inhalt der test.mtl-Datei ebenfalls erhalten durch
