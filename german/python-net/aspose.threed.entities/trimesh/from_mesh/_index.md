...
type: docs
weight: 70
url: /de/python-net/aspose.threed.entities/trimesh/from_mesh/
is_root: false
---
...
# Definiere eine Vertex-Deklaration als {FVector3 Position; FVector3 Normal; FVector2 UV}
...
# Konvertiere ein Mesh zu einem Tri-Mesh unter Verwendung des angegebenen Speicherlayouts
mesh = Sphere().to_mesh()
triMesh = TriMesh.from_mesh(vd, mesh)
# Speichere es in einen Stream, 115 Vertices * 32 Bytes pro Vertex
with open("output.bin", "wb") as stream:
    triMesh.write_vertices_to(stream)
    # Speichere Indizes als ushort in den Stream, 504 Indizes * 2 Bytes pro Index
