---
title: HShape
second_title: Aspose.3D für Node.js via Java API-Referenz
description: 
type: docs

url: /de/nodejs-java/aspose.threed/hshape/
---
## HShape class

Der HShape liefert die definierenden Parameter einer 'H'- oder 'I'-Form.


## Methoden

### constructor{#constructor}

| Name | Beschreibung |
| --- | --- |
| constructor() | Konstruktor von HShape |

 **Result:**



---


### getOverallDepth{#getOverallDepth}

| Name | Beschreibung |
| --- | --- |
| getOverallDepth() | Liest oder setzt die Ausdehnung der Tiefe. |

 **Result:**



---


### setOverallDepth{#setOverallDepth}

| Name | Beschreibung |
| --- | --- |
| setOverallDepth(value) | Liest oder setzt die Ausdehnung der Tiefe. |

 **Result:**



---


### getBottomFlangeWidth{#getBottomFlangeWidth}

| Name | Beschreibung |
| --- | --- |
| getBottomFlangeWidth() | Liest oder setzt die Ausdehnung der Breite. |

 **Result:**



---


### setBottomFlangeWidth{#setBottomFlangeWidth}

| Name | Beschreibung |
| --- | --- |
| setBottomFlangeWidth(value) | Liest oder setzt die Ausdehnung der Breite. |

 **Result:**



---


### getTopFlangeWidth{#getTopFlangeWidth}

| Name | Beschreibung |
| --- | --- |
| getTopFlangeWidth() | Liest oder setzt die Breite des oberen Flansches. |

 **Result:**



---


### setTopFlangeWidth{#setTopFlangeWidth}

| Name | Beschreibung |
| --- | --- |
| setTopFlangeWidth(value) | Liest oder setzt die Breite des oberen Flansches. |

 **Result:**



---


### getTopFlangeThickness{#getTopFlangeThickness}

| Name | Beschreibung |
| --- | --- |
| getTopFlangeThickness() | Liest oder setzt die Dicke des oberen Flansches. |

 **Result:**



---


### setTopFlangeThickness{#setTopFlangeThickness}

| Name | Beschreibung |
| --- | --- |
| setTopFlangeThickness(value) | Liest oder setzt die Dicke des oberen Flansches. |

 **Result:**



---


### getTopFlangeEdgeRadius{#getTopFlangeEdgeRadius}

| Name | Beschreibung |
| --- | --- |
| getTopFlangeEdgeRadius() | Liest oder setzt den Radius der unteren Kanten des oberen Flansches. |

 **Result:**



---


### setTopFlangeEdgeRadius{#setTopFlangeEdgeRadius}

| Name | Beschreibung |
| --- | --- |
| setTopFlangeEdgeRadius(value) | Liest oder setzt den Radius der unteren Kanten des oberen Flansches. |

 **Result:**



---


### getTopFlangeFilletRadius{#getTopFlangeFilletRadius}

| Name | Beschreibung |
| --- | --- |
| getTopFlangeFilletRadius() | Liest oder setzt den Radius der Rundung zwischen dem Steg und dem oberen Flansch. |

 **Result:**



---


### setTopFlangeFilletRadius{#setTopFlangeFilletRadius}

| Name | Beschreibung |
| --- | --- |
| setTopFlangeFilletRadius(value) | Liest oder setzt den Radius der Rundung zwischen dem Steg und dem oberen Flansch. |

 **Result:**



---


### getBottomFlangeThickness{#getBottomFlangeThickness}

| Name | Beschreibung |
| --- | --- |
| getBottomFlangeThickness() | Liest oder setzt die Flanschdicke der H-Form. |

 **Result:**



---


### setBottomFlangeThickness{#setBottomFlangeThickness}

| Name | Beschreibung |
| --- | --- |
| setBottomFlangeThickness(value) | Liest oder setzt die Flanschdicke der H-Form. |

 **Result:**



---


### getWebThickness{#getWebThickness}

| Name | Beschreibung |
| --- | --- |
| getWebThickness() | Liest oder setzt die Dicke des Stegs der H-Form. |

 **Result:**



---


### setWebThickness{#setWebThickness}

| Name | Beschreibung |
| --- | --- |
| setWebThickness(value) | Liest oder setzt die Dicke des Stegs der H-Form. |

 **Result:**



---


### getBottomFlangeFilletRadius{#getBottomFlangeFilletRadius}

| Name | Beschreibung |
| --- | --- |
| getBottomFlangeFilletRadius() | Liest oder setzt den Radius der Rundung zwischen dem Steg und dem unteren Flansch. |

 **Result:**



---


### setBottomFlangeFilletRadius{#setBottomFlangeFilletRadius}

| Name | Beschreibung |
| --- | --- |
| setBottomFlangeFilletRadius(value) | Liest oder setzt den Radius der Rundung zwischen dem Steg und dem unteren Flansch. |

 **Result:**



---


### getBottomFlangeEdgeRadius{#getBottomFlangeEdgeRadius}

| Name | Beschreibung |
| --- | --- |
| getBottomFlangeEdgeRadius() | Liest oder setzt den Radius der oberen Kanten des unteren Flansches. |

 **Result:**



---


### setBottomFlangeEdgeRadius{#setBottomFlangeEdgeRadius}

| Name | Beschreibung |
| --- | --- |
| setBottomFlangeEdgeRadius(value) | Liest oder setzt den Radius der oberen Kanten des unteren Flansches. |

 **Result:**



---


### getParentNodes{#getParentNodes}

| Name | Beschreibung |
| --- | --- |
| getParentNodes() | Gibt alle übergeordneten Knoten zurück, ein Entity kann für Geometrie-Instanziierung an mehrere übergeordnete Knoten angehängt werden. Die Knoten. |

 **Result:**



---


### getExcluded{#getExcluded}

| Name | Beschreibung |
| --- | --- |
| getExcluded() | Gibt an oder legt fest, ob dieses Entity beim Exportieren ausgeschlossen wird. |

 **Result:**



---


### setExcluded{#setExcluded}

| Name | Beschreibung |
| --- | --- |
| setExcluded(value) | Gibt an oder legt fest, ob dieses Entity beim Exportieren ausgeschlossen wird. |

 **Result:**



---


### getParentNode{#getParentNode}

| Name | Beschreibung |
| --- | --- |
| getParentNode() | Gibt den ersten übergeordneten Knoten zurück oder legt ihn fest; wenn der erste übergeordnete Knoten gesetzt wird, wird dieses Entity von anderen übergeordneten Knoten getrennt. Der übergeordnete Knoten. |

 **Result:**



---


### setParentNode{#setParentNode}

| Name | Beschreibung |
| --- | --- |
| setParentNode(value) | Gibt den ersten übergeordneten Knoten zurück oder legt ihn fest; wenn der erste übergeordnete Knoten gesetzt wird, wird dieses Entity von anderen übergeordneten Knoten getrennt. Der übergeordnete Knoten. |

 **Result:**



---


### getScene{#getScene}

| Name | Beschreibung |
| --- | --- |
| getScene() | Liefert die Szene, zu der dieses Objekt gehört |

 **Result:**



---


### getName{#getName}

| Name | Beschreibung |
| --- | --- |
| getName() | Gibt den Namen zurück oder legt ihn fest. Der Name. |

 **Result:**



---


### setName{#setName}

| Name | Beschreibung |
| --- | --- |
| setName(value) | Gibt den Namen zurück oder legt ihn fest. Der Name. |

 **Result:**



---


### getProperties{#getProperties}

| Name | Beschreibung |
| --- | --- |
| getProperties() | Liefert die Sammlung aller Eigenschaften. |

 **Result:**



---


### getExtent{#getExtent}

| Name | Beschreibung |
| --- | --- |
| getExtent() | Ermittelt die Ausdehnung in x- und y-Richtung. |

 **Result:**
Vector2


---


### getEntityRendererKey{#getEntityRendererKey}

| Name | Beschreibung |
| --- | --- |
| getEntityRendererKey() | Liefert den Schlüssel des Entitäts-Renderers, der im Renderer registriert ist |

 **Result:**
EntityRendererKey


---


### getBoundingBox{#getBoundingBox}

| Name | Beschreibung |
| --- | --- |
| getBoundingBox() | Liefert die Begrenzungsbox der aktuellen Entität in ihrem Objekt-Raum-Koordinatensystem. |

 **Result:**
EntityRendererKey


---


### removeProperty{#removeProperty}

| Name | Beschreibung |
| --- | --- |
| removeProperty(property) | Entfernt eine dynamische Eigenschaft. |

 **Parameters:**

| Name | Typ | Beschreibung |
| --- | --- | --- |
| Eigenschaft | Property | Welche Eigenschaft zu entfernen ist |

 **Result:**
boolean


---


### removeProperty{#removeProperty}

| Name | Beschreibung |
| --- | --- |
| removeProperty(property) | Entferne die angegebene Eigenschaft, die durch ihren Namen identifiziert wird |

 **Parameters:**

| Name | Typ | Beschreibung |
| --- | --- | --- |
| propert | String | null |

 **Result:**
boolean


---


### getProperty{#getProperty}

| Name | Beschreibung |
| --- | --- |
| getProperty(property) | Liefere den Wert der angegebenen Eigenschaft |

 **Parameters:**

| Name | Typ | Beschreibung |
| --- | --- | --- |
| Eigenschaft | String | Eigenschaftsname |

 **Result:**
Object


---


### setProperty{#setProperty}

| Name | Beschreibung |
| --- | --- |
| setProperty(property, value) | Setzt den Wert der angegebenen Eigenschaft |

 **Parameters:**

| Name | Typ | Beschreibung |
| --- | --- | --- |
| Eigenschaft | String | Eigenschaftsname |
| Wert | Object | Der Wert der Eigenschaft |

 **Result:**
Object


---


### findProperty{#findProperty}

| Name | Beschreibung |
| --- | --- |
| findProperty(propertyName) | Findet die Eigenschaft. Sie kann eine dynamische Eigenschaft sein (erstellt durch CreateDynamicProperty/SetProperty) oder eine native Eigenschaft (identifiziert durch ihren Namen) |

 **Parameters:**

| Name | Typ | Beschreibung |
| --- | --- | --- |
| propertyName | String | Eigenschaftsname. |

 **Result:**
Property


---



