...
type: docs
weight: 70
url: /it/python-net/aspose.threed.entities/trimesh/from_mesh/
is_root: false
---
...
# Definisci una dichiarazione di vertice come {FVector3 Position; FVector3 Normal; FVector2 UV}
...
# converti una mesh in tri-mesh usando il layout di memoria specificato
mesh = Sphere().to_mesh()
triMesh = TriMesh.from_mesh(vd, mesh)
# salvalo in uno stream, 115 vertici * 32 byte per vertice
with open("output.bin", "wb") as stream:
    triMesh.write_vertices_to(stream)
    # salva gli indici come ushort nello stream, 504 indici * 2 byte per indice
