---
title: "Texture"
second_title: "Aspose.3D 用于 Node.js 通过 Java API 参考"
description: 
type: docs

url: /zh/nodejs-java/aspose.threed/texture/
---
## Texture class

此类定义来自外部文件的纹理。


## 方法

### constructor{#constructor}

| 名称 | 描述 |
| --- | --- |
| constructor() | 初始化 Texture 类的新实例。 |

 **Result:**



---


### constructor_overload{#constructor_overload}

| 名称 | 描述 |
| --- | --- |
| constructor_overload(name) | 初始化 Texture 类的新实例。 |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| name | 字符串 | 名称 |

 **Result:**



---


### getEnableMipMap{#getEnableMipMap}

| 名称 | 描述 |
| --- | --- |
| getEnableMipMap() | 获取或设置此纹理是否启用 mipmap |

 **Result:**



---


### setEnableMipMap{#setEnableMipMap}

| 名称 | 描述 |
| --- | --- |
| setEnableMipMap(value) | 获取或设置此纹理是否启用 mipmap |

 **Result:**



---


### getContent{#getContent}

| 名称 | 描述 |
| --- | --- |
| getContent() | 获取或设置纹理的二进制内容。嵌入的纹理内容是可选的，如果缺少，用户应从外部文件加载纹理。 |

 **Result:**



---


### setContent{#setContent}

| 名称 | 描述 |
| --- | --- |
| setContent(value) | 获取或设置纹理的二进制内容。嵌入的纹理内容是可选的，如果缺少，用户应从外部文件加载纹理。 |

 **Result:**



---


### getFileName{#getFileName}

| 名称 | 描述 |
| --- | --- |
| getFileName() | 获取或设置关联的纹理文件。文件的名称。 |

 **Result:**



---


### setFileName{#setFileName}

| 名称 | 描述 |
| --- | --- |
| setFileName(value) | 获取或设置关联的纹理文件。文件的名称。 |

 **Result:**



---


### getAlpha{#getAlpha}

| 名称 | 描述 |
| --- | --- |
| getAlpha() | 获取或设置纹理的默认 alpha 值。当 AlphaSource 为 AlphaSource.PIXEL_ALPHA 时有效。默认值为 1.0，有效值范围在 0 到 1 之间。 |

 **Result:**



---


### setAlpha{#setAlpha}

| 名称 | 描述 |
| --- | --- |
| setAlpha(value) | 获取或设置纹理的默认 alpha 值。当 AlphaSource 为 AlphaSource.PIXEL_ALPHA 时有效。默认值为 1.0，有效值范围在 0 到 1 之间。 |

 **Result:**



---


### getAlphaSource{#getAlphaSource}

| 名称 | 描述 |
| --- | --- |
| getAlphaSource() | 获取或设置纹理是否定义 alpha 通道。默认值为 AlphaSource.NONE。属性的值是 AlphaSource 整数常量。 |

 **Result:**



---


### setAlphaSource{#setAlphaSource}

| 名称 | 描述 |
| --- | --- |
| setAlphaSource(value) | 获取或设置纹理是否定义 alpha 通道。默认值为 AlphaSource.NONE。属性的值是 AlphaSource 整数常量。 |

 **Result:**



---


### getWrapModeU{#getWrapModeU}

| 名称 | 描述 |
| --- | --- |
| getWrapModeU() | 获取或设置 U 方向的纹理环绕模式。属性的值是 WrapMode 整数常量。 |

 **Result:**



---


### setWrapModeU{#setWrapModeU}

| 名称 | 描述 |
| --- | --- |
| setWrapModeU(value) | 获取或设置 U 方向的纹理环绕模式。属性的值是 WrapMode 整数常量。 |

 **Result:**



---


### getWrapModeV{#getWrapModeV}

| 名称 | 描述 |
| --- | --- |
| getWrapModeV() | 获取或设置 V 方向的纹理环绕模式。属性的值是 WrapMode 整数常量。 |

 **Result:**



---


### setWrapModeV{#setWrapModeV}

| 名称 | 描述 |
| --- | --- |
| setWrapModeV(value) | 获取或设置 V 方向的纹理环绕模式。属性的值是 WrapMode 整数常量。 |

 **Result:**



---


### getWrapModeW{#getWrapModeW}

| 名称 | 描述 |
| --- | --- |
| getWrapModeW() | 获取或设置 W 方向的纹理环绕模式。属性的值是 WrapMode 整数常量。 |

 **Result:**



---


### setWrapModeW{#setWrapModeW}

| 名称 | 描述 |
| --- | --- |
| setWrapModeW(value) | 获取或设置 W 方向的纹理环绕模式。属性的值是 WrapMode 整数常量。 |

 **Result:**



---


### getMinFilter{#getMinFilter}

| 名称 | 描述 |
| --- | --- |
| getMinFilter() | 获取或设置用于缩小的过滤器。属性的值是 TextureFilter 整数常量。 |

 **Result:**



---


### setMinFilter{#setMinFilter}

| 名称 | 描述 |
| --- | --- |
| setMinFilter(value) | 获取或设置用于缩小的过滤器。属性的值是 TextureFilter 整数常量。 |

 **Result:**



---


### getMagFilter{#getMagFilter}

| 名称 | 描述 |
| --- | --- |
| getMagFilter() | 获取或设置放大过滤器。属性的值是 TextureFilter 整数常量。 |

 **Result:**



---


### setMagFilter{#setMagFilter}

| 名称 | 描述 |
| --- | --- |
| setMagFilter(value) | 获取或设置放大过滤器。属性的值是 TextureFilter 整数常量。 |

 **Result:**



---


### getMipFilter{#getMipFilter}

| 名称 | 描述 |
| --- | --- |
| getMipFilter() | 获取或设置 mip 级别采样的过滤器。属性的值是 TextureFilter 整数常量。 |

 **Result:**



---


### setMipFilter{#setMipFilter}

| 名称 | 描述 |
| --- | --- |
| setMipFilter(value) | 获取或设置 mip 级别采样的过滤器。属性的值是 TextureFilter 整数常量。 |

 **Result:**



---


### getUVRotation{#getUVRotation}

| 名称 | 描述 |
| --- | --- |
| getUVRotation() | 获取或设置纹理的旋转 |

 **Result:**



---


### setUVRotation{#setUVRotation}

| 名称 | 描述 |
| --- | --- |
| setUVRotation(value) | 获取或设置纹理的旋转 |

 **Result:**



---


### getUVScale{#getUVScale}

| 名称 | 描述 |
| --- | --- |
| getUVScale() | 获取或设置 UV 缩放。UV 缩放。 |

 **Result:**



---


### setUVScale{#setUVScale}

| 名称 | 描述 |
| --- | --- |
| setUVScale(value) | 获取或设置 UV 缩放。UV 缩放。 |

 **Result:**



---


### getUVTranslation{#getUVTranslation}

| 名称 | 描述 |
| --- | --- |
| getUVTranslation() | 获取或设置 UV 平移。UV 平移。 |

 **Result:**



---


### setUVTranslation{#setUVTranslation}

| 名称 | 描述 |
| --- | --- |
| setUVTranslation(value) | 获取或设置 UV 平移。UV 平移。 |

 **Result:**



---


### getName{#getName}

| 名称 | 描述 |
| --- | --- |
| getName() | 获取或设置名称。名称。 |

 **Result:**



---


### setName{#setName}

| 名称 | 描述 |
| --- | --- |
| setName(value) | 获取或设置名称。名称。 |

 **Result:**



---


### getProperties{#getProperties}

| 名称 | 描述 |
| --- | --- |
| getProperties() | 获取所有属性的集合。 |

 **Result:**



---


### setTranslation{#setTranslation}

| 名称 | 描述 |
| --- | --- |
| setTranslation(u, v) | 设置 UV 平移。 |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| u | 数字 | U。 |
| v | 数字 | V. |

 **Result:**



---


### setScale{#setScale}

| 名称 | 描述 |
| --- | --- |
| setScale(u, v) | 设置 UV 缩放。 |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| u | 数字 | U。 |
| v | 数字 | V. |

 **Result:**



---


### setRotation{#setRotation}

| 名称 | 描述 |
| --- | --- |
| setRotation(u, v) | 设置 UV 旋转。 |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| u | 数字 | U。 |
| v | 数字 | V. |

 **Result:**



---


### removeProperty{#removeProperty}

| 名称 | 描述 |
| --- | --- |
| removeProperty(property) | 移除动态属性。 |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| property | Property | 要移除哪个属性 |

 **Result:**
boolean


---


### removeProperty{#removeProperty}

| 名称 | 描述 |
| --- | --- |
| removeProperty(property) | 移除按名称标识的指定属性 |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| propert | 字符串 | null |

 **Result:**
boolean


---


### getProperty{#getProperty}

| 名称 | 描述 |
| --- | --- |
| getProperty(property) | 获取指定属性的值 |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| property | 字符串 | 属性名称 |

 **Result:**
对象


---


### setProperty{#setProperty}

| 名称 | 描述 |
| --- | --- |
| setProperty(property, value) | 设置指定属性的值 |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| property | 字符串 | 属性名称 |
| 值 | 对象 | 属性的值 |

 **Result:**
对象


---


### findProperty{#findProperty}

| 名称 | 描述 |
| --- | --- |
| findProperty(propertyName) | 查找属性。它可以是动态属性（由 CreateDynamicProperty/SetProperty 创建）或本机属性（通过其名称标识） |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| propertyName | 字符串 | 属性名称。 |

 **Result:**
Property


---



