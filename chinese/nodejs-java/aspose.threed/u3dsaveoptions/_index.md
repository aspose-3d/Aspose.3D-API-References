---
title: "U3dSaveOptions"
second_title: "Aspose.3D 用于 Node.js 通过 Java API 参考"
description: 
type: docs

url: /zh/nodejs-java/aspose.threed/u3dsaveoptions/
---
## U3dSaveOptions class

通用3D的保存选项


## 方法

### constructor{#constructor}

| 名称 | 描述 |
| --- | --- |
| constructor() | U3dSaveOptions 的构造函数 |

 **Result:**



---


### getFlipCoordinateSystem{#getFlipCoordinateSystem}

| 名称 | 描述 |
| --- | --- |
| getFlipCoordinateSystem() | 获取或设置在导入/导出期间是否翻转控制点/法线的坐标系。 |

 **Result:**



---


### setFlipCoordinateSystem{#setFlipCoordinateSystem}

| 名称 | 描述 |
| --- | --- |
| setFlipCoordinateSystem(value) | 获取或设置在导入/导出期间是否翻转控制点/法线的坐标系。 |

 **Result:**



---


### getMeshCompression{#getMeshCompression}

| 名称 | 描述 |
| --- | --- |
| getMeshCompression() | 获取或设置是否启用网格数据压缩。 |

 **Result:**



---


### setMeshCompression{#setMeshCompression}

| 名称 | 描述 |
| --- | --- |
| setMeshCompression(value) | 获取或设置是否启用网格数据压缩。 |

 **Result:**



---


### getExportNormals{#getExportNormals}

| 名称 | 描述 |
| --- | --- |
| getExportNormals() | 获取或设置是否导出法线数据。 |

 **Result:**



---


### setExportNormals{#setExportNormals}

| 名称 | 描述 |
| --- | --- |
| setExportNormals(value) | 获取或设置是否导出法线数据。 |

 **Result:**



---


### getExportTextureCoordinates{#getExportTextureCoordinates}

| 名称 | 描述 |
| --- | --- |
| getExportTextureCoordinates() | 获取或设置是否导出纹理坐标。 |

 **Result:**



---


### setExportTextureCoordinates{#setExportTextureCoordinates}

| 名称 | 描述 |
| --- | --- |
| setExportTextureCoordinates(value) | 获取或设置是否导出纹理坐标。 |

 **Result:**



---


### getExportVertexDiffuse{#getExportVertexDiffuse}

| 名称 | 描述 |
| --- | --- |
| getExportVertexDiffuse() | 获取或设置是否导出顶点的漫反射颜色。 |

 **Result:**



---


### setExportVertexDiffuse{#setExportVertexDiffuse}

| 名称 | 描述 |
| --- | --- |
| setExportVertexDiffuse(value) | 获取或设置是否导出顶点的漫反射颜色。 |

 **Result:**



---


### getExportVertexSpecular{#getExportVertexSpecular}

| 名称 | 描述 |
| --- | --- |
| getExportVertexSpecular() | 获取或设置是否导出顶点的高光颜色。 |

 **Result:**



---


### setExportVertexSpecular{#setExportVertexSpecular}

| 名称 | 描述 |
| --- | --- |
| setExportVertexSpecular(value) | 获取或设置是否导出顶点的高光颜色。 |

 **Result:**



---


### getEmbedTextures{#getEmbedTextures}

| 名称 | 描述 |
| --- | --- |
| getEmbedTextures() | 将外部纹理嵌入 U3D 文件，默认值为 false。 |

 **Result:**



---


### setEmbedTextures{#setEmbedTextures}

| 名称 | 描述 |
| --- | --- |
| setEmbedTextures(value) | 将外部纹理嵌入 U3D 文件，默认值为 false。 |

 **Result:**



---


### getExportTextures{#getExportTextures}

| 名称 | 描述 |
| --- | --- |
| getExportTextures() | 尝试将场景中使用的纹理复制到输出目录。 |

 **Result:**



---


### setExportTextures{#setExportTextures}

| 名称 | 描述 |
| --- | --- |
| setExportTextures(value) | 尝试将场景中使用的纹理复制到输出目录。 |

 **Result:**



---


### getFileFormat{#getFileFormat}

| 名称 | 描述 |
| --- | --- |
| getFileFormat() | 获取当前保存/加载选项中指定的文件格式。 |

 **Result:**



---


### getEncoding{#getEncoding}

| 名称 | 描述 |
| --- | --- |
| getEncoding() | 获取或设置基于文本文件的默认编码。默认值为 null，表示导入器/导出器将决定使用哪种编码。 |

 **Result:**



---


### getFileSystem{#getFileSystem}

| 名称 | 描述 |
| --- | --- |
| getFileSystem() | 允许用户处理在加载/保存期间如何管理外部依赖项。 |

 **Result:**



---


### setFileSystem{#setFileSystem}

| 名称 | 描述 |
| --- | --- |
| setFileSystem(value) | 允许用户处理在加载/保存期间如何管理外部依赖项。 |

 **Result:**



---


### getLookupPaths{#getLookupPaths}

| 名称 | 描述 |
| --- | --- |
| getLookupPaths() | 某些文件（如 OBJ）依赖外部文件，查找路径允许 Aspose.3D 查找并加载外部文件。 |

 **Result:**



---


### getFileName{#getFileName}

| 名称 | 描述 |
| --- | --- |
| getFileName() | 导出/导入场景的文件名。此项可选，但在序列化外部资源（如 OBJ 的材质）时很有用。 |

 **Result:**



---


### setFileName{#setFileName}

| 名称 | 描述 |
| --- | --- |
| setFileName(value) | 导出/导入场景的文件名。此项可选，但在序列化外部资源（如 OBJ 的材质）时很有用。 |

 **Result:**



---



