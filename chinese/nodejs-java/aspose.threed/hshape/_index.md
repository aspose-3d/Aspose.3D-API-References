---
title: "HShape"
second_title: "Aspose.3D 用于 Node.js 通过 Java API 参考"
description: 
type: docs

url: /zh/nodejs-java/aspose.threed/hshape/
---
## HShape class

HShape 提供了 'H' 或 'I' 形状的定义参数。


## 方法

### constructor{#constructor}

| 名称 | 描述 |
| --- | --- |
| constructor() | HShape 的构造函数 |

 **Result:**



---


### getOverallDepth{#getOverallDepth}

| 名称 | 描述 |
| --- | --- |
| getOverallDepth() | 获取或设置深度的范围。 |

 **Result:**



---


### setOverallDepth{#setOverallDepth}

| 名称 | 描述 |
| --- | --- |
| setOverallDepth(value) | 获取或设置深度的范围。 |

 **Result:**



---


### getBottomFlangeWidth{#getBottomFlangeWidth}

| 名称 | 描述 |
| --- | --- |
| getBottomFlangeWidth() | 获取或设置宽度的范围。 |

 **Result:**



---


### setBottomFlangeWidth{#setBottomFlangeWidth}

| 名称 | 描述 |
| --- | --- |
| setBottomFlangeWidth(value) | 获取或设置宽度的范围。 |

 **Result:**



---


### getTopFlangeWidth{#getTopFlangeWidth}

| 名称 | 描述 |
| --- | --- |
| getTopFlangeWidth() | 获取或设置上法兰的宽度。 |

 **Result:**



---


### setTopFlangeWidth{#setTopFlangeWidth}

| 名称 | 描述 |
| --- | --- |
| setTopFlangeWidth(value) | 获取或设置上法兰的宽度。 |

 **Result:**



---


### getTopFlangeThickness{#getTopFlangeThickness}

| 名称 | 描述 |
| --- | --- |
| getTopFlangeThickness() | 获取或设置上法兰的厚度。 |

 **Result:**



---


### setTopFlangeThickness{#setTopFlangeThickness}

| 名称 | 描述 |
| --- | --- |
| setTopFlangeThickness(value) | 获取或设置上法兰的厚度。 |

 **Result:**



---


### getTopFlangeEdgeRadius{#getTopFlangeEdgeRadius}

| 名称 | 描述 |
| --- | --- |
| getTopFlangeEdgeRadius() | 获取或设置上法兰下缘的半径。 |

 **Result:**



---


### setTopFlangeEdgeRadius{#setTopFlangeEdgeRadius}

| 名称 | 描述 |
| --- | --- |
| setTopFlangeEdgeRadius(value) | 获取或设置上法兰下缘的半径。 |

 **Result:**



---


### getTopFlangeFilletRadius{#getTopFlangeFilletRadius}

| 名称 | 描述 |
| --- | --- |
| getTopFlangeFilletRadius() | 获取或设置腹板与上法兰之间的圆角半径。 |

 **Result:**



---


### setTopFlangeFilletRadius{#setTopFlangeFilletRadius}

| 名称 | 描述 |
| --- | --- |
| setTopFlangeFilletRadius(value) | 获取或设置腹板与上法兰之间的圆角半径。 |

 **Result:**



---


### getBottomFlangeThickness{#getBottomFlangeThickness}

| 名称 | 描述 |
| --- | --- |
| getBottomFlangeThickness() | 获取或设置 H 形状的法兰厚度。 |

 **Result:**



---


### setBottomFlangeThickness{#setBottomFlangeThickness}

| 名称 | 描述 |
| --- | --- |
| setBottomFlangeThickness(value) | 获取或设置 H 形状的法兰厚度。 |

 **Result:**



---


### getWebThickness{#getWebThickness}

| 名称 | 描述 |
| --- | --- |
| getWebThickness() | 获取或设置 H 形状腹板的厚度。 |

 **Result:**



---


### setWebThickness{#setWebThickness}

| 名称 | 描述 |
| --- | --- |
| setWebThickness(value) | 获取或设置 H 形状腹板的厚度。 |

 **Result:**



---


### getBottomFlangeFilletRadius{#getBottomFlangeFilletRadius}

| 名称 | 描述 |
| --- | --- |
| getBottomFlangeFilletRadius() | 获取或设置腹板与底法兰之间圆角的半径。 |

 **Result:**



---


### setBottomFlangeFilletRadius{#setBottomFlangeFilletRadius}

| 名称 | 描述 |
| --- | --- |
| setBottomFlangeFilletRadius(value) | 获取或设置腹板与底法兰之间圆角的半径。 |

 **Result:**



---


### getBottomFlangeEdgeRadius{#getBottomFlangeEdgeRadius}

| 名称 | 描述 |
| --- | --- |
| getBottomFlangeEdgeRadius() | 获取或设置底法兰上缘的半径。 |

 **Result:**



---


### setBottomFlangeEdgeRadius{#setBottomFlangeEdgeRadius}

| 名称 | 描述 |
| --- | --- |
| setBottomFlangeEdgeRadius(value) | 获取或设置底法兰上缘的半径。 |

 **Result:**



---


### getParentNodes{#getParentNodes}

| 名称 | 描述 |
| --- | --- |
| getParentNodes() | 获取所有父节点，实体可以附加到多个父节点以实现几何实例化。 |

 **Result:**



---


### getExcluded{#getExcluded}

| 名称 | 描述 |
| --- | --- |
| getExcluded() | 获取或设置在导出期间是否排除此实体。 |

 **Result:**



---


### setExcluded{#setExcluded}

| 名称 | 描述 |
| --- | --- |
| setExcluded(value) | 获取或设置在导出期间是否排除此实体。 |

 **Result:**



---


### getParentNode{#getParentNode}

| 名称 | 描述 |
| --- | --- |
| getParentNode() | 获取或设置第一个父节点，如果设置了第一个父节点，则此实体将从其他父节点分离。父节点。 |

 **Result:**



---


### setParentNode{#setParentNode}

| 名称 | 描述 |
| --- | --- |
| setParentNode(value) | 获取或设置第一个父节点，如果设置了第一个父节点，则此实体将从其他父节点分离。父节点。 |

 **Result:**



---


### getScene{#getScene}

| 名称 | 描述 |
| --- | --- |
| getScene() | 获取此对象所属的场景 |

 **Result:**



---


### getName{#getName}

| 名称 | 描述 |
| --- | --- |
| getName() | 获取或设置名称。名称。 |

 **Result:**



---


### setName{#setName}

| 名称 | 描述 |
| --- | --- |
| setName(value) | 获取或设置名称。名称。 |

 **Result:**



---


### getProperties{#getProperties}

| 名称 | 描述 |
| --- | --- |
| getProperties() | 获取所有属性的集合。 |

 **Result:**



---


### getExtent{#getExtent}

| 名称 | 描述 |
| --- | --- |
| getExtent() | 获取 x 和 y 维度的范围 |

 **Result:**
Vector2


---


### getEntityRendererKey{#getEntityRendererKey}

| 名称 | 描述 |
| --- | --- |
| getEntityRendererKey() | 获取在渲染器中注册的实体渲染器的键 |

 **Result:**
EntityRendererKey


---


### getBoundingBox{#getBoundingBox}

| 名称 | 描述 |
| --- | --- |
| getBoundingBox() | 获取当前实体在其对象空间坐标系中的边界框。 |

 **Result:**
EntityRendererKey


---


### removeProperty{#removeProperty}

| 名称 | 描述 |
| --- | --- |
| removeProperty(property) | 移除动态属性。 |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| property | Property | 要移除哪个属性 |

 **Result:**
boolean


---


### removeProperty{#removeProperty}

| 名称 | 描述 |
| --- | --- |
| removeProperty(property) | 移除按名称标识的指定属性 |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| propert | 字符串 | null |

 **Result:**
boolean


---


### getProperty{#getProperty}

| 名称 | 描述 |
| --- | --- |
| getProperty(property) | 获取指定属性的值 |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| property | 字符串 | 属性名称 |

 **Result:**
对象


---


### setProperty{#setProperty}

| 名称 | 描述 |
| --- | --- |
| setProperty(property, value) | 设置指定属性的值 |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| property | 字符串 | 属性名称 |
| 值 | 对象 | 属性的值 |

 **Result:**
对象


---


### findProperty{#findProperty}

| 名称 | 描述 |
| --- | --- |
| findProperty(propertyName) | 查找属性。它可以是动态属性（由 CreateDynamicProperty/SetProperty 创建）或本机属性（通过其名称标识） |

 **Parameters:**

| 名称 | 类型 | 描述 |
| --- | --- | --- |
| propertyName | 字符串 | 属性名称。 |

 **Result:**
Property


---



