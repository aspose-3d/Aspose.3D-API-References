...
description: "Aspose.ThreeD.Node 类。表示场景图中的一个元素。场景图是由 Node 对象组成的树。该类内部包含树管理服务。请注意，Aspose.3D SDK 不会验证构建的场景图的有效性。调用方有责任确保不会在节点层次结构中生成循环图。除了树管理外，该类还定义了描述对象在场景中位置所需的所有属性。这些信息包括基本的平移、旋转和缩放属性，以及用于枢轴、限制和 IK 关节的更高级选项，如刚度和阻尼。首次创建时，Node 对象为空，即它是一个没有任何图形表示、仅包含位置信息的对象。在此状态下，它可用于表示节点树结构中的父节点，但功能有限。此类对象的常规用法是为其添加一个实体，以专门化节点，参见 Entity。实体本身是一个对象，并连接到 Node。这也意味着同一实体可以在多个节点之间共享。Camera、Light、Mesh 等都是实体，并且都派生自基类 Entity。"
type: docs
weight: 1630
...
| 名称 | 描述 |
| --- | --- |
| [AssetInfo](../../aspose.threed/node/assetinfo/) { get; set; } | 每个节点的资产信息 |
| [ChildNodes](../../aspose.threed/node/childnodes/) { get; } | 获取子节点。 |
| [Entities](../../aspose.threed/node/entities/) { get; } | 获取所有节点实体。 |
| [Entity](../../aspose.threed/node/entity/) { get; set; } | 获取或设置附加到此节点的第一个实体，如果设置，将清除其他实体。 |
| [Excluded](../../aspose.threed/node/excluded/) { get; set; } | 获取或设置在导出时是否排除此节点及所有子节点/实体。 |
| [GlobalTransform](../../aspose.threed/node/globaltransform/) { get; } | 获取全局变换。 |
| [Material](../../aspose.threed/node/material/) { get; set; } | 获取或设置与此节点关联的第一个材质，如果设置，将清除其他材质。 |
...
| 名称 | 描述 |
| --- | --- |
| [Accept](../../aspose.threed/node/accept/)(NodeVisitor) | 遍历所有子后代节点（包括当前节点），并将访问者调用于该节点。访问者可以通过返回 false 来中断遍历。 |
...
| [CreateChildNode](../../aspose.threed/node/createchildnode/#createchildnode_2)(string) | 创建一个具有给定节点名称的新子节点 |
| [CreateChildNode](../../aspose.threed/node/createchildnode/#createchildnode_3)(string, Entity) | 创建一个具有给定节点名称的新子节点 |
| [CreateChildNode](../../aspose.threed/node/createchildnode/#createchildnode_4)(string, Entity, Material) | 创建一个具有给定节点名称的新子节点，并附加指定的实体和材质 |
